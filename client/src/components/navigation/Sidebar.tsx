...
import { X, BookOpen, Lightbulb, Award, Newspaper, BrainCircuit, User, LayoutDashboard, ClipboardList, Wrench, Library, Gamepad2, GraduationCap } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, toggleSidebar }) => {
  ...
  const menuItems = [
    ...
    { path: '/relaxing-area', name: 'Relaxing Area', icon: <Gamepad2 size={20} /> },
    { path: '/news', name: 'Academic News', icon: <Newspaper size={20} /> },
    { path: '/ai-assistant', name: 'AI Assistant', icon: <BrainCircuit size={20} /> },
    { path: '/opportunities', name: 'Opportunities', icon: <Award size={20} /> },
    { path: '/teacher', name: 'Teacher Dashboard', icon: <GraduationCap size={20} /> },
    ...
    >
      <div className="flex items-center justify-between h-16 px-4 bg-primary-900">
        <div className="flex items-center">
          ...
          <span className="ml-2 text-xl font-semibold text-white font-serif">ScholarHub</span>
        </div>
        <button 
      ...
      <div className="absolute bottom-0 left-0 right-0 p-4">
        <div className="bg-primary-700 rounded-md p-4 text-primary-100">
          <h4 className="text-sm font-medium mb-2 text-accent-300">Need Help?</h4>
          <p className="text-xs mb-3">Access tutorials and helpful resources for using ScholarHub effectively.</p>
